...
let testTitle = ['google', 'reddit', 'facebook']
var testText = [`What’s covered in these terms
...
Live Policies: These policies apply to all content broadcast to Facebook through Facebook Live.
...
function textAnalyze(name) {
  ...
  for(let x = 0; x < terms.length; x++) {
    processedTerms[x] = {
      term: terms[x][0],
      weight: terms[x][1],
      length: terms[x][0].length,
      freq: doc.getTermFrequency(terms[x][0])
    };
  }
...
class App extends React.Component {
  ...
  clickReddit = () => {
    let terms = textAnalyze("reddit");
    this.setState({ showReddit: true, 
                    terms: terms
    });
  };
  clickGoogle = () => {
    let terms = textAnalyze("google");
    this.setState({ showGoogle: true, 
                    terms: terms
    });
  };
  clickFacebook = () => {
    let terms = textAnalyze("facebook");
